...
class BertForMultiLabelSequenceClassification(nn.Module):
    def __init__(self, bert_pretrained_model_dir=None):
        ...
        self.tgt_labels = torch.LongTensor(torch.from_numpy(np.array([idx for idx in range(21)])))
        self.groupfc1 = GroupWiseLinear(21, 768)
    ...
    def forward(self,
        ...
        prediction_res = torch.stack(prediction_scores, dim=1).reshape(-1, 21)

        if labels is not None:
            loss_decoder = FocalDiceLoss()(prediction_res.view(-1, 21), labels.view(-1, 21))
...
class Decoder(nn.Module):
    def __init__(self, d_model):
        ...
        self.tgt_emb = nn.Embedding(21, self.d_model)
